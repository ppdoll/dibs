...
예약금 만료·순위 확정·메일 발송은 스케줄 잡이 진행시킨다. 화면을 쓰고 있으면 요청에 얹혀
60초마다 알아서 돌지만(§7), 기다리지 않고 **지금 당장** 돌리고 싶을 때가 있다.
...
스케줄 잡이 지나가며 따라잡는다. 테스터는 실제 시간이 흐르기를 기다릴 수 없으므로 **직접 호출한다.**

**대부분의 경우 이거 하나면 된다** — 잡 8개를 올바른 순서로 전부 돌린다:

```bash
curl -X POST http://localhost:3001/api/cron/tick \
  -H "Authorization: Bearer dev-only-cron-secret-32chars-min"
```

`Tasks: Run Task` → **`크론 전부 한 번씩 때리기`** 가 같은 일을 하고 결과를 표로 찍어 준다.

> 참고: 운영에서는 손으로 때릴 일이 없다. 들어오는 요청에 얹힌 `TickInterceptor` 가
> 60초마다 알아서 굴린다(docs/DEPLOY.md §4). 로컬에서도 화면을 쓰고 있으면 마찬가지다 —
> 아래는 "지금 당장" 돌리고 싶을 때의 이야기다.

아래는 **개별 잡만 따로** 확인하고 싶을 때 쓴다.
전부 `Authorization: Bearer $CRON_SECRET` 이 필요하다.
